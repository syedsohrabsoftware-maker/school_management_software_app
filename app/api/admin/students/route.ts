...
export async function GET(req: Request) {
  try {
    ...
    let data = rows.map((r) => ({
      ...
      rollNo: r.rollNo,
      photo: photoUrl(r.photo), // ✅ CONVERTED
        ...
        if (relevantLinks.length > 0) {
          ...
          for (const link of relevantLinks) {
            ...
            folioSiblings[link.folioNo].push({
              ...
              photo: photoUrl(sibStudent.photo), // ✅ ADDED
              rollNo: sibStudent.rollNo ?? null,
              relationToMain: link.relationToMain,
              class: sibStudent.classId
                ? { id: sibStudent.classId, className: sibStudent.className }
                : null,
              section: sibStudent.sectionId
                ? { id: sibStudent.sectionId, sectionName: sibStudent.sectionName }
                : null,
            });
          }
